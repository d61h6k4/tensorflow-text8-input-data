{-# LANGUAGE LambdaCase #-}
-- | Downloads the Text8 data set and packages them as data files.
-- Package create like tensorflow-mnist-input-data.
module Main where
...
downloadingDataFiles =
  hooks {confHook = \gh@(g, _) c -> downloadFiles g >> confHook hooks gh c}
  where
...
httpDownload url outFile = do
  let uri =
        fromMaybe (error ("Can't be: invalid URI " ++ url)) (URI.parseURI url)
  ...
  HTTP.getResponseCode result >>= \case
    ...
    s ->
      error
        ("Failed to download " ++
         url ++ " error code " ++ show s ++ helpfulMessage)
  ...
  when (hash /= computed) $
    error
      ("Incorrect checksum for " ++
       filePath ++
       "\nexpected " ++ hash ++ "\ncomputed " ++ computed ++ helpfulMessage)
